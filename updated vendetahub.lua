...
--==================================================
--// AUTO GOURDS
--// Charges mana, waits 2 seconds, finds the largest available
--// gourd the player owns, fires it, then repeats until none remain.
--==================================================
...
local function FindOwnedGourd()
    ...
    for _,GourdName in ipairs(GourdNames) do
        for _,Container in ipairs({CharacterNow, Backpack}) do
            if Container then
                local Exact = Container:FindFirstChild(GourdName, true)
                if Exact then
                    return GourdName
                end
            end
        ...
        while AutoGourds and GUI.Parent do
            ...
            if not GourdName then
                ...
                pcall(function()
                    StarterGui:SetCore("SendNotification", {
                        Title = "Vendetta Hub",
                        Text = "No gourds found / gourd cycle finished.",
                        Duration = 3
                    })
                end)
                break
            ...
            -- Give the game time to consume/break the gourd before the next cycle.
            task.wait(2)
        end
