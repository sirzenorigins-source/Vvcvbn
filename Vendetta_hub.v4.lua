...
local AutoGourdsButton = Button(FarmPage,"Auto Gourds   •   OFF",132)
local RedeemCodesButton = Button(FarmPage,"Redeem All Codes",176)
...
--==================================================
--// SMART STREAMING ITEM SEARCH
--// Moves through map scan points so streamed areas can load.
--==================================================

local StarterGui = game:GetService("StarterGui")
local SmartSearchBusy = false
local SmartSearchTween = nil
...
local function SmartSearchTarget(Finder,Label,OnFound)
    if SmartSearchBusy then
        Notify("Vendetta Hub", "A map search is already running.", 2)
        return
    end

    SmartSearchBusy = true
    Notify("Vendetta Hub", "Searching for "..Label.."...", 2)

    local SearchPoints = {
        Vector3.new(-1316.6278076171875, 1037.992431640625, 363.0509338378906),
        Vector3.new(-1563.84619140625, 1048.1646728515625, -255.4888153076172),
        Vector3.new(-2196.5888671875, 997.2844848632812, -1391.724609375),
        Vector3.new(-2546.173583984375, 827.7539672851562, -267.8199157714844),
        Vector3.new(-2533.99755859375, 827.6522827148438, 551.3883666992188),
        Vector3.new(312.53424072265625, 918.849365234375, 341.8284912109375),
        Vector3.new(-269.2065124511719, 1019.6824951171875, -94.4324951171875),
        Vector3.new(-531.0517578125, 950.0924072265625, 751.8784790039062),
        Vector3.new(-1212.78515625, 1092.41357421875, 995.0526123046875),
        Vector3.new(-2111.99462890625, 1076.794677734375, 1320.8599853515625),
        Vector3.new(-470.40008544921875, 1022.5936279296875, 1618.2305908203125),
        Vector3.new(151.46131896972656, 932.6013793945312, 1073.98388671875),
    }

    local FoundPart = Finder()

    local function TryFound()
        local Part = Finder()
    ...
    for Index,Position in ipairs(SearchPoints) do
        if FoundPart or not GUI.Parent or not HRP then break end

        if SmartSearchTween then
            pcall(function() SmartSearchTween:Cancel() end)
            SmartSearchTween = nil
        end

        -- Visit each supplied location once, in the exact order provided.
        local Distance = (Position - HRP.Position).Magnitude
        local Duration = math.max(Distance / 300, 1.5)

        SmartSearchTween = TS:Create(
            HRP,
            TweenInfo.new(Duration, Enum.EasingStyle.Linear),
            {CFrame = CFrame.new(Position + Vector3.new(0,3,0))}
        )
        SmartSearchTween:Play()
        SmartSearchTween.Completed:Wait()
        SmartSearchTween = nil

        -- Give StreamingEnabled time to replicate the area before checking it.
        task.wait(1)
        TryFound()
    end
    ...
    if FoundPart then
        Notify("Vendetta Hub", Label.." found!", 3)
        if OnFound then OnFound(FoundPart) end
    else
        Notify("Vendetta Hub", Label.." not found after checking all search locations.", 4)
    end
end
...
IceTweenButton.MouseButton1Click:Connect(function()
    SmartSearchTarget(function()
        return GetIceShardPart(GetIceShard())
    end,"Ice Shard",function(Part)
...
FluteTween.MouseButton1Click:Connect(function()
    SmartSearchTarget(FindFlute,"Flute",function(Part)
        TweenTo(Part)
    end)
end)

EarringsTween.MouseButton1Click:Connect(function()
    SmartSearchTarget(FindEarrings,"Earrings",function(Part)
        TweenTo(Part)
    end)
end)
...
--==================================================
--// AUTO GOURDS
--// Based on the Auto-Gourd logic from the supplied Pastefy script.
--// It checks the player's owned gourd tools, charges mana, waits,
--// then uses the highest available gourd until none remain.
--==================================================
...
local function FindOwnedGourd()
    local CharacterNow = LP.Character
    if not CharacterNow then return nil end

    -- Match the supplied script's priority: only actual Tool instances.
    for _,GourdName in ipairs(GourdNames) do
        local Tool = CharacterNow:FindFirstChild(GourdName)
...
local function StartAutoGourds()
    ...
    task.spawn(function()
        ...
        Remote = Remote and Remote:FindFirstChild("remote")

        while AutoGourds and GUI.Parent do
            if not Remote then
                AutoGourds = false
                break
            end

            -- Same mana-charge event used elsewhere in Vendetta Hub.
            pcall(function()
            ...
            local GourdName = FindOwnedGourd()
            if not GourdName then
                AutoGourds = false
                AutoGourdsButton.Text = "Auto Gourds   •   OFF"
                SetToggleVisual(AutoGourdsButton, false)
                Notify("Vendetta Hub", "No gourds found / gourd cycle finished.", 3)
                break
            end

            -- The supplied script fires the gourd name through the same remote.
            pcall(function()
