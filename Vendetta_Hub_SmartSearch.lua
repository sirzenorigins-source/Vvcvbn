...
local function SmartSearchTarget(Finder,Label,OnFound)
    ...
    if not Start then
        ...
        Notify("Vendetta Hub", Label.." not found.", 3)
        return
    end

    -- Spiral/grid scan. Each point gives StreamingEnabled a chance to
    -- replicate that region before we search the client hierarchy again.
    local Step = 350
    local Rings = 10
    local WaitAtChunk = 0.45
    ...
    if not FoundPart then
        for Ring = 1,Rings do
            local Points = {}
            local Limit = Ring * Step

            for X = -Limit,Limit,Step do
                table.insert(Points, Vector3.new(Start.X + X, Start.Y, Start.Z - Limit))
                table.insert(Points, Vector3.new(Start.X + X, Start.Y, Start.Z + Limit))
            end
            for Z = -Limit + Step,Limit - Step,Step do
                table.insert(Points, Vector3.new(Start.X - Limit, Start.Y, Start.Z + Z))
                table.insert(Points, Vector3.new(Start.X + Limit, Start.Y, Start.Z + Z))
            end
            ...
            for _,Position in ipairs(Points) do
                ...
                if SmartSearchTween then
                    SmartSearchTween:Cancel()
                    SmartSearchTween = nil
                end

                local Distance = (Position - HRP.Position).Magnitude
                local Duration = math.clamp(Distance / 700, 0.25, 0.8)
    ...
    if FoundPart then
        ...
        Notify("Vendetta Hub", Label.." not found.", 4)
    end
end
...
local function ActivateStrongAttack()
    local DashWithNoDelay = game:GetService("ReplicatedStorage"):FindFirstChild("DashWithNoDelay")
    if not DashWithNoDelay then return false end
    local ok = pcall(function()
        DashWithNoDelay:InvokeServer("StrongAttack")
    end)
    return ok
end
...
ComboLeave.MouseButton1Click:Connect(function()
    ...
    if Remote then
        ...
        if HRP then
            local Back = -HRP.CFrame.LookVector * 25
...
local function SetFlying(State)
    ...
    FlyConnection = RS.RenderStepped:Connect(function(dt)
        ...
        -- CFrame flight + non-colliding character parts keeps high-speed CFly smooth through walls.
        ...
        if Move.Magnitude > 0 then
            Horizontal = Forward*Move:Dot(Forward) + Right*Move:Dot(Right)
            if Horizontal.Magnitude > 1 then Horizontal=Horizontal.Unit end
        end
        local Y = UpHeld and 1 or (DownHeld and -1 or Vertical)
        local Direction = Horizontal + Vector3.new(0,Y,0)
        if Direction.Magnitude > 1 then Direction=Direction.Unit end
        local Step = Direction * FlySpeed * math.min(dt,1/30)
